...
class TokenTypeToStringTest : public ::testing::TestWithParam<std::pair<TokenType, std::string>> {
};

TEST_P(TokenTypeToStringTest, TokenTypeToString) {
  const auto [type, expected] = GetParam();

  EXPECT_EQ(to_string(type), expected);
}

INSTANTIATE_TEST_SUITE_P(
  TokenTypes,
  TokenTypeToStringTest,
  ::testing::Values(
    std::make_pair(TokenType::AND, "AND"),
...
TEST(TokenTest, ToString) {
  Token token{TokenType::ID, "x", 1, 2};
  
  EXPECT_EQ(token.to_string(), "[1:2] ID, x");
}