...
void DiagnosticsManager::addError(const Twine& message, SMRange location,
                                  ArrayRef<SMRange> ranges,
                                  ArrayRef<SMFixIt> fixits) {
  SMDiagnostic diagnostic = m_source_manager.GetMessage(
      location.Start, SourceMgr::DK_Error, message, ranges, fixits);
